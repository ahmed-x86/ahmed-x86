
# 👋 Hi, I'm Ahmed (ahmed-x86)

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=ahmed-x86&label=Profile%20Views&color=0e75b6&style=for-the-badge"/>
</p>

<p align="center">
  <img src="https://skillicons.dev/icons?i=linux,arch,bash,cpp,git,neovim,vscode"/>
</p>

---

# 🧑‍💻 About Me

🐧 **Linux Enthusiast** obsessed with customizing and pushing Linux to its limits.  
⚙️ I enjoy automation, scripting, and building tools that simplify development workflows.  
🧠 Interested in **Cyber Security, and system internals**.  

Currently working on improving developer workflows on **Linux and Windows**.

🌐 **Check out my personal website:** [ahmed-x86.github.io](https://ahmed-x86.github.io/)
---


# 🐧 My Linux Setup

<p align="center">
  <img src="https://raw.githubusercontent.com/ahmed-x86/hyprland_dotfiles/main/screenshot_2026-02-07_19-23-34.png" width="32%" />
  <img src="https://raw.githubusercontent.com/ahmed-x86/hyprland_dotfiles/main/screenshot_2026-02-07_10-21-48.png" width="32%" />
  <img src="https://raw.githubusercontent.com/ahmed-x86/hyprland_dotfiles/main/screenshot_2026-02-07_17-03-54.png" width="32%" />
</p>


**OS:** Arch Linux (BTW)  
**WM:** Hyprland  
**Bar:** Waybar  
**Editor:** Neovim  
**Terminal:** Foot / Kitty  

I spend a lot of time **building custom tools**.

---

# 🚀 Featured Projects

## 🧠 Assembly Development Environment
📦 **Make VS Code support Assembly on Windows and Linux easily**

🔗 https://github.com/ahmed-x86/asm

Features:

- Automatic NASM installation
- Windows + Linux support
- One command installation
- Designed for students learning Assembly
## for linux

```bash
bash <(curl -sSL [https://raw.githubusercontent.com/ahmed-x86/asm/main/install-linux.sh](https://raw.githubusercontent.com/ahmed-x86/asm/main/install-linux.sh))
```
### for windows
```powershell
irm https://raw.githubusercontent.com/ahmed-x86/asm/refs/heads/main/install-windows.ps1 | iex
```
### for windows 7
```powershell
irm https://raw.githubusercontent.com/ahmed-x86/asm/refs/heads/main/install-windows7.ps1 | iex
```


---

## 🎨 Hyprland Dotfiles

🔗 https://github.com/ahmed-x86/hyprland_dotfiles

My personal **Hyprland rice configuration**.

Includes:

- Waybar configuration
- Keybindings
- Window rules
- Custom themes
- Optimized workflow

Perfect if you want a **clean Arch + Hyprland setup**.

---

## 🔋 Mobile Charge Indicator (Waybar)

🔗 https://github.com/ahmed-x86/mobile_charge_indicator_waybar

A **Waybar module** that shows the **charging status of your phone**.

Features:

- Detects phone connection
- Shows charging state
- Clean integration with Waybar

---

## ⚡ Useful Scripts Collection

🔗 https://github.com/ahmed-x86/my_scripts

A collection of useful scripts I wrote for:

- Linux automation
- development tools
- system utilities
- workflow improvements

---

# 🧰 Tech Stack

### Languages
<p>
<img src="https://skillicons.dev/icons?i=bash,cpp"/>
</p>

### Tools
<p>
<img src="https://skillicons.dev/icons?i=git,neovim,vscode,linux"/>
</p>

### os
<p>
<img src="https://skillicons.dev/icons?i=arch"/>
</p>

---

# 💻 Operating Systems Experience

<p align="center">

<img src="https://img.shields.io/badge/Arch_Linux-1793D1?style=for-the-badge&logo=arch-linux&logoColor=white"/>
<img src="https://img.shields.io/badge/Debian-A81D33?style=for-the-badge&logo=debian&logoColor=white"/>
<img src="https://img.shields.io/badge/Fedora-51A2DA?style=for-the-badge&logo=fedora&logoColor=white"/>
<img src="https://img.shields.io/badge/Ubuntu-E9433F?style=for-the-badge&logo=ubuntu&logoColor=white"/>
<img src="https://img.shields.io/badge/Linux_Mint-87CF3E?style=for-the-badge&logo=linux-mint&logoColor=white"/>
<img src="https://img.shields.io/badge/Manjaro-35BF5C?style=for-the-badge&logo=manjaro&logoColor=white"/>
<img src="https://img.shields.io/badge/CachyOS-0080FF?style=for-the-badge&logo=arch-linux&logoColor=white"/>
<img src="https://img.shields.io/badge/Windows_10-0078D6?style=for-the-badge&logo=windows&logoColor=white"/>
<img src="https://img.shields.io/badge/Windows_11-0078D6?style=for-the-badge&logo=windows&logoColor=white"/>

</p>

---


# 🎯 (Principles)

<p align="center">
  
> **"Free Software, Free Society."**  
> — Richard Stallman

</p>

---

# ⚡ Fun Fact


I once thought Arch Linux was impossible to use.

Now...

I use Arch BTW.


---

<p align="center">
🔥 Powered by Linux • Built with passion • Driven by curiosity
</p>


---

